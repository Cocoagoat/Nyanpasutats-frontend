export const redirectBoxContent = [
  {
    title: "Affinity Finder",
    description:
      "Find the person with the highest (and lowest) affinity to you on MAL.*",
    link: "/affinity",
  },
  {
    title: "Anime Recommendations",
    description:
      "Have your shit taste analyzed by a neural network, which will then recommend shows that closely fit it. Lorem ipsum something something",
    link: "/recs",
  },
  {
    title: "Seasonal Cards",
    description:
      "Create a mini-infographic that will summarize how a certain season went for you.",
    link: "/seasonal",
  },
  {
    title: "History of Ratings",
    description:
      "See how your favorite anime was rated throughout time - has it been consistent or did it fall of a cliff since it stopped airing?",
    link: "/history",
  },
];
