...
export const RecommendationAnswers = {
  ...
  each tag (and some common combinations of tags) from Anilist's
   tag database. </br></br> This "tag affinity" is calculated as a number for each tag, combined
    that with a show's actual "tag percentage" as they are listed on Anilist (For example, one show could have the "Band" tag with a 96% next to it, while another would have it with a 60% 
    - the significance of each tag is decided by users), and then this combination is fed  to a neural network trained to guess the score you (a user with "tag affinities" X,Y,Z) would give some show with tags A,B,C.
</br> </br>
